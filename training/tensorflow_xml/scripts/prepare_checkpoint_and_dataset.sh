...
cd "${OBJ_DET_DIR}"
python generate_tfrecord.py -x ${DATASET_DIR}/train -l ${DATASET_DIR}/dataset_label_map.pbtxt -o ${DATASET_DIR}/dataset_train.record
python generate_tfrecord.py -x ${DATASET_DIR}/test -l ${DATASET_DIR}/dataset_label_map.pbtxt -o ${DATASET_DIR}/dataset_test.record
python update_test_examples.py \
       --test_record_path="${DATASET_DIR}/dataset_test.record" \
       --ckpt_dir="${CKPT_DIR}"
